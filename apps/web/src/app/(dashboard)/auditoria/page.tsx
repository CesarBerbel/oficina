  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        <Input placeholder="Filtrar por módulo (ex.: service-orders)" value={moduleF}
          onChange={(e) => { setModuleF(e.target.value); setPage(1); }} className="sm:w-64" />
        <Input placeholder="Filtrar por ação (ex.: STATUS_CHANGE)" value={actionF}
          onChange={(e) => { setActionF(e.target.value); setPage(1); }} className="sm:w-64" />
      </div>
              ...
              rows.map((r) => (
                <TableRow key={r.id}>
                  ...
                  <TableCell>
                    ...
                    <div><Badge variant="secondary">{r.action}</Badge></div>
                  </TableCell>
...
{JSON.stringify({ antes: r.before, depois: r.after }, null, 2)}
      ...
      {meta && meta.totalPages > 1 && (
        <div className="flex items-center justify-between text-sm">
          <span className="text-muted-foreground">{meta.total} evento(s) · página {meta.page} de {meta.totalPages}</span>
