            ...
            {isLoading ? (
              ...
              posts.map((p) => (
                <TableRow key={p.id}>
                  ...
                  <TableCell>
                    <Badge variant={VARIANT[p.status]}>{BLOG_STATUS_LABELS[p.status]}</Badge>
                  </TableCell>
                    ...
                    {canWrite && (
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          {p.status === 'RASCUNHO' ? (
                            <DropdownMenuItem onClick={() => handlePublish(p, true)}>
                              <Send className="size-4" /> Publicar
                            </DropdownMenuItem>
                          ) : (
                            <DropdownMenuItem onClick={() => handlePublish(p, false)}>
                              <Undo2 className="size-4" /> Voltar a rascunho
                            </DropdownMenuItem>
                          )}
                          {p.status === 'PUBLICADO' && (
                            <DropdownMenuItem asChild>
                              <Link href={`/site/blog/${p.slug}`} target="_blank">
