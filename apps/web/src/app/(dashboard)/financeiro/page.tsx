    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground">
            Contas a receber, contas a pagar, baixas e fluxo de caixa.
          </p>
        </div>
        {canWrite && (
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <Kpi
          ...
          value={summary?.receivableOpen ?? 0}
          loading={loadingSummary}
          tone="positive"
        />
        <Kpi
          title="A pagar em aberto"
          value={summary?.payableOpen ?? 0}
          loading={loadingSummary}
          tone="negative"
        />
        <Kpi
          title="Fluxo realizado 30 dias"
          value={summary?.netCashFlow ?? 0}
          loading={loadingSummary}
          tone={(summary?.netCashFlow ?? 0) >= 0 ? 'positive' : 'negative'}
        />
        <Kpi
      ...
      <div className="rounded-xl border bg-card p-4">
        <div className="mb-3 flex flex-wrap gap-2">
          ...
          >
            ...
            {Object.values(FinancialEntryType).map((option) => (
              <option key={option} value={option}>
                {FINANCIAL_ENTRY_TYPE_LABELS[option]}
              </option>
            ))}
          </Select>
          <Select
          ...
          >
            ...
            {Object.values(FinancialEntryStatus).map((option) => (
              <option key={option} value={option}>
                {FINANCIAL_ENTRY_STATUS_LABELS[option]}
              </option>
            ))}
          </Select>
        </div>
      ...
      <PayFinancialDialog
        entry={payEntry}
        open={!!payEntry}
        onOpenChange={(open) => !open && setPayEntry(null)}
      />
    </div>
  );
}
  ...
  return (
    <TableRow>
      ...
      <TableCell className={entry.overdue ? 'font-medium text-destructive' : ''}>
        ...
        {entry.overdue && <span className="block text-xs">vencido</span>}
      </TableCell>
      <TableCell>
        ...
        {canSettle && (
          <div className="flex justify-end gap-1">
            <Button size="sm" variant="outline" onClick={onPay}>
              <CheckCircle2 className="size-4" /> Baixa
            </Button>
            {entry.paidAmount === 0 && (
              <Button
                size="icon"
