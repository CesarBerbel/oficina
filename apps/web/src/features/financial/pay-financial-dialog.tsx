...
export function PayFinancialDialog({
  entry,
  open,
  onOpenChange,
}: {
  ...
  const [form, setForm] = useState<{
    ...
    amount: '',
    method: FinancialPaymentMethod.PIX,
  ...
  async function onSubmit(e: React.FormEvent) {
    e.preventDefault();
    const parsed = payFinancialEntrySchema.safeParse({
      ...form,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            {entry.description} · saldo {formatCurrency(entry.remainingAmount)}
          </DialogDescription>
        </DialogHeader>
        <form className="space-y-4" onSubmit={onSubmit}>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-1.5">
              ...
              <Select
                value={form.method}
                onChange={(e) =>
                  setForm((f) => ({ ...f, method: e.target.value as FinancialPaymentMethod }))
                }
              >
                {Object.values(FinancialPaymentMethod).map((method) => (
                  <option key={method} value={method}>
                    {FINANCIAL_PAYMENT_METHOD_LABELS[method]}
                  </option>
                ))}
              </Select>
            </div>
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={pay.isPending}>
              {pay.isPending && <CarLoader className="size-4 animate-spin" />} Registrar
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
