...
const empty = { name: '', event: 'MANUAL', channel: 'WHATSAPP', body: '', active: true, autoSend: false };
...
export function TemplateFormDialog({
  ...
  open: boolean; onOpenChange: (o: boolean) => void; template?: MessageTemplateDto | null;
}) {
  ...
  useEffect(() => {
    if (!open) return;
    setForm(template ? {
      name: template.name, event: template.event, channel: template.channel,
      body: template.body, active: template.active, autoSend: template.autoSend,
    } : empty);
    setErrors({});
  }, [open, template]);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      if (isEdit) { await update.mutateAsync(parsed.data); toast.success('Template atualizado'); }
      else { await create.mutateAsync(parsed.data as never); toast.success('Template criado'); }
      onOpenChange(false);
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-1.5">
              ...
              <Select value={form.event} onChange={(e) => set('event', e.target.value)} className="w-full">
                {MESSAGE_EVENTS.map((ev) => <option key={ev} value={ev}>{MESSAGE_EVENT_LABELS[ev]}</option>)}
              </Select>
            </div>
            <div className="space-y-1.5">
          ...
          <div className="flex gap-6">
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" className="size-4" checked={form.active} onChange={(e) => set('active', e.target.checked)} /> Ativo
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" className="size-4" checked={form.autoSend} onChange={(e) => set('autoSend', e.target.checked)} /> Envio automático no evento
            </label>
          </div>
          <DialogFooter>
