...
import { ReceptionAlertsPanel, normalizeReceptionAlerts } from './components/reception-alerts-panel';
...
export function ReceptionPage() {
  ...
  useEffect(() => {
    ...
    if (alerts.checkedInWithoutOs.length > 0) {
      toast.info(`${alerts.checkedInWithoutOs.length} cliente(s) chegaram e ainda precisam virar OS.`);
      return;
    }
  ...
  return (
    ...
    >
      <aside className={cn('min-h-0 rounded-xl border bg-card', fullScreenView && 'overflow-hidden')}>
        <div className="border-b p-4">
          ...
          <ReceptionAlertsPanel alerts={alerts} onSelect={(leadId) => selectLead(leadId, 'agenda')} />
          ...
          <div className={cn('mt-4 grid gap-2', fullScreenView ? 'lg:grid-cols-[1fr_16rem]' : 'space-y-2')}>
        ...
        <div
          className={cn(
            view === 'kanban' && 'h-[calc(100dvh-23rem)] min-h-[30rem] overflow-x-auto overflow-y-hidden p-3',
            view === 'agenda' && 'h-[calc(100dvh-23rem)] min-h-[30rem] overflow-y-auto p-3',
            view !== 'agenda' && view !== 'kanban' && 'max-h-[calc(100dvh-22rem)] overflow-y-auto p-2',
          )}
        >
          {isLoading ? (
            ...
            <p className="py-10 text-center text-sm text-muted-foreground">Nenhum atendimento encontrado.</p>
          ) : view === 'kanban' ? (
            <ReceptionKanban leads={leads} selectedId={selectedId} now={now} onSelect={(id) => selectLead(id)} />
          ) : view === 'agenda' ? (
            <ReceptionAgendaView leads={visibleLeads} selectedId={selectedId} onSelect={(id) => selectLead(id)} />
          ) : (
            <ReceptionList leads={visibleLeads} selectedId={selectedId} onSelect={(id) => selectLead(id)} />
          )}
        </div>
      </aside>
