...
export class GlobalSearchService {
  ...
  async search(
    ...
    const [customers, vehicles, serviceOrders, leads, parts, services] =
    ...
    const results: GlobalSearchResultDto[] = [
      ...customers.map((customer) => ({
        ...
        description: compact([
          `${customer._count.vehicles} veículo(s)`,
        ...
        score:
          20 +
          scoreText(
            [customer.name, customer.email, customer.document, customer.phone, customer.whatsapp],
            q,
          ),
      ...
      ...vehicles.map((vehicle) => ({
        ...
        description: compact([
          vehicle.modelYear,
          vehicle.color,
          vehicle.currentKm ? `${vehicle.currentKm} km` : null,
        ]),
        badge: 'Veículo',
        ...
        score:
          ...
          Math.max(
            scoreText(
              [vehicle.plate, vehicle.manufacturer, vehicle.model, vehicle.customer.name],
              q,
            ),
      ...
      ...leads.map((lead) => ({
        ...
        score:
          14 +
          scoreText([lead.name, lead.phone, lead.email, lead.plate, lead.vehicle, lead.message], q),
      ...
      ...parts.map((part) => ({
        ...
        title: part.name,
        subtitle: compact([
          part.sku ? `Código ${part.sku}` : null,
          part.ncm ? `NCM ${part.ncm}` : null,
          part.brand,
        ]),
        description: compact([
          part.category,
          `${Number(part.stocks[0]?.currentStock ?? 0)} ${part.unit}`,
        ]),
        ...
        score:
          12 +
          scoreText(
            [part.name, part.sku, part.ncm, part.brand, part.category, part.description],
            q,
          ),
